(function() {
  var LinkChecker, jsdom, l, p, request, root, u;
  ...
  root.LinkChecker = LinkChecker = (function() {
        ...
        if (!error && response.statusCode === 200) {
          ...
          if (ref.valid_link(url.link)) {
            try {
              ...
              return ref.log(err, LinkChecker.LOG_CRITICAL);
            }
          }
        } else {
          if (!error) {
    ...
    LinkChecker.prototype.valid_link = function(link) {
      var valid;
      link = u.parse(link);
      valid = true;
  ...
  root.run = function() {
    var lc;
    p.version('0.0.1').option('-u, --url [url]', 'URL to check').option('-v, --verbose', 'Verbose').parse(process.argv);
