(function() {
  ...
  root.LinkChecker = LinkChecker = (function() {
    ...
    LinkChecker.prototype.start = function(callback) {
      ...
      if (this.base !== void 0 && this.base.length > 0 && LinkChecker.REGEX_EMAIL.test(this.base)) {
    ...
    LinkChecker.prototype.check_end = function() {
      if (this.queued.length === 0 && !this.finished) {
        ...
        this.log("Queued " + this.queued.length + " - Processed " + this.processed.length);
        return this.log("Queued " + this.queued);
      }
    };
        ...
        if (!error && response.statusCode === 200) {
          ...
          if (link.valid_process_link(_this.base)) {
            try {
              ...
              _this.log("****************************** (jsdom)", LinkChecker.LOG_CRITICAL);
              _this.log(err, LinkChecker.LOG_CRITICAL);
              return _this.remove_from_queue(link);
            }
          } else {
            return _this.remove_from_queue(link);
          ...
          if (!error) {
            _this.log("" + response.statusCode + " at " + (link.to_string()), LinkChecker.LOG_CRITICAL);
            link.code = response.statusCode;
          } else {
