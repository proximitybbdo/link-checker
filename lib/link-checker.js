(function() {
        ...
        return link.request = request(request_o, function(error, response, body) {
          if (!error && response.statusCode === 200) {
            ...
            if (link.valid_process_link(_this.base)) {
              return _this.process_dom_page(link, body);
            } else {
              return _this.remove_from_queue(link);
            }
          } else {
  ...
  */
  ...
  root.run = function() {
    ...
    p.version("" + package.name + " " + package.version).option('-u, --url [url]', 'URL to check').option('-f, --full', 'Full output, default is only http codes >= 400').option('-p, --piped', 'Piped output, default is json').option('-v, --verbose', 'Verbose').parse(process.argv);
